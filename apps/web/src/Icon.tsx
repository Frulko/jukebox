...
/**
 * Glyphs a theme draws differently.
 *
 * The rest of this file is one set for the whole app, which was right while the
 * app had one look. It has four, and an icon is as much a part of a skin as a
 * colour is: the same house means "home" in the iTunes 8 chrome and in a dark
 * modern one, but not with the same weight, the same corners or the same fill.
 * A theme names only what it wants changed and inherits everything else — the
 * same rule the colour tokens follow, so a new skin is still one block and no
 * component is touched.
 *
 * Studio is the one that needed it: heavier, rounder, filled — a set drawn for
 * a dark surface, where the hairlines of a 2008 toolbar disappear.
 */
...
export function Icon({ name, size = 12, className = '' }: { name: keyof typeof PATHS | string; size?: number; className?: string }) {
  ...
  const d = THEMED[theme]?.[name] ?? PATHS[name] ?? ''
  return (
